...
public interface RegMsgRepository extends JpaRepository<RegMsg,Long> {
    ...
    List<RegMsg> selectLimit(@Param("brand") String brand,@Param("color") String color,@Param("ctype") String ctype,@Param("i")int i,@Param("j")int j);

    @Query(value = "select * from regmsg where brand = :brand and color = :color and ctype = :ctype ",nativeQuery = true)
    List<RegMsg> select(@Param("brand") String brand,@Param("color") String color,@Param("ctype") String ctype);
    ...
    @Query(value = "select * from regmsg where brand = :brand and color = :color and ctype = :ctype",nativeQuery = true)
    List<RegMsg> getPageByBrandColorCtypeAll(@Param("brand")String brand, @Param("color")String color, @Param("ctype")String ctype);
    @Query(value = "select * from regmsg where brand = :brand and color = :color and ctype = :ctype limit :i,:j ",nativeQuery = true)
